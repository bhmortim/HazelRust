...
pub struct IList<T> {
    ...
    /// Cached partition id (-1 = not yet computed). An IList lives on a single
    /// partition derived from its immutable name, so it's computed once instead
    /// of re-serializing the name on every op.
