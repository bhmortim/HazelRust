...
//! including both successful commits and rollbacks.
//!
//! Run with: `cargo run --example transactions`
//!
//! Requires a Hazelcast cluster running on localhost:5701.

use hazelcast_client::{ClientConfig, HazelcastClient, TransactionOptions};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Initialize accounts
    let accounts = client.get_map::<String, i64>("accounts");
    ...
    println!("\n--- Transaction 1: Transfer $200 from Alice to Bob ---");
    {
        ...
        let tx_accounts = txn.get_map::<String, i64>("accounts")?;
        let tx_log = txn.get_queue::<String>("transaction-log")?;
