...
fn compute_result(
    ...
    let p99 = if !sorted.is_empty() {
        sorted[(n * 0.99) as usize % sorted.len()]
    } else {
        0.0
    };
    BenchmarkResult {
        client: "rust".into(),
        data_structure: ds.into(),
        operation: op.into(),
        scale,
        concurrency: conc,
        value_size_bytes: val,
        near_cache: false,
        ops_per_sec: ops_sec,
        p50_ms: p50,
        p99_ms: p99,
...
async fn bench_map_op(
    ...
    if needs_seed {
        for i in 0..scale {
            let _ = map.put(format!("k{i}"), v.clone()).await;
        }
    }
    ...
    for i in 0..20 {
        ...
        match op {
            "put" => {
                let _ = map.put(k, v.clone()).await;
            }
            "get" => {
                let _ = map.get(&format!("k{}", i % scale.max(1))).await;
            }
            _ => {
                let _ = map.put(k, v.clone()).await;
            }
        ...
        match op {
            ...
            "set_ttl" => {
                measure(scale, |i| {
                    let m = &map;
                    Box::pin(async move {
                        m.set_ttl(&format!("k{i}"), Duration::from_secs(300))
                            .await
                            .map(|_| ())
                            .map_err(|e| Box::new(e) as _)
                    })
            ...
            "keys" => {
                let iters = 10.max(1000 / scale.max(1));
                let mut lats = Vec::new();
                let mut errs = 0;
                for _ in 0..iters {
                    let s = Instant::now();
                    match map.keys().await {
                        Ok(_) => lats.push(s.elapsed().as_micros() as f64 / 1000.0),
                        Err(_) => errs += 1,
                    }
                }
            ...
            "put_all(10)" => {
                ...
                for b in 0..nb {
                    let batch: HashMap<String, String> = (0..10)
                        .map(|i| (format!("k{}", b * 10 + i), val(vsize)))
                        .collect();
                    let s = Instant::now();
                    match map.put_all(batch).await {
                        Ok(_) => lats.push(s.elapsed().as_micros() as f64 / 1000.0),
                        Err(_) => errs += 1,
                    }
                }
            ...
            "put_all(100)" => {
                ...
                for b in 0..nb {
                    let batch: HashMap<String, String> = (0..100)
                        .map(|i| (format!("k{}", b * 100 + i), val(vsize)))
                        .collect();
                    let s = Instant::now();
                    match map.put_all(batch).await {
                        Ok(_) => lats.push(s.elapsed().as_micros() as f64 / 1000.0),
            ...
            "get_all(10)" => {
                ...
                let mut errs = 0;
                for b in 0..nb {
                    let keys: Vec<String> = (0..10).map(|i| format!("k{}", b * 10 + i)).collect();
                    let s = Instant::now();
            ...
            "get_all(100)" => {
                ...
                let mut errs = 0;
                for b in 0..nb {
                    let keys: Vec<String> = (0..100).map(|i| format!("k{}", b * 100 + i)).collect();
                    let s = Instant::now();
                    match map.get_all(&keys).await {
                        Ok(_) => lats.push(s.elapsed().as_micros() as f64 / 1000.0),
                        Err(_) => errs += 1,
            ...
            "mixed" => {
                ...
                for i in 0..scale {
                    let pct = i % 10;
                    let k = format!("k{}", i % scale);
                    let s = Instant::now();
                    let ok = if pct < 7 {
                        map.get(&k).await.is_ok()
                    } else if pct < 9 {
                        map.put(k, val(64)).await.is_ok()
                    } else {
                        map.delete(&k).await.is_ok()
                    };
                    if ok {
                        lats.push(s.elapsed().as_micros() as f64 / 1000.0);
                    } else {
                        errs += 1;
                    }
                }
                    ...
                    let ok = match o.as_str() {
                        ...
                        "mixed" => {
                            ...
                            if pct < 7 {
                                ...
                                m.put(k, val(64)).await.is_ok()
                            } else {
                                m.delete(&k).await.is_ok()
                            }
                        }
                        _ => false,
                    };
...
async fn bench_queue_op(client: &HazelcastClient, op: &str, scale: usize) -> BenchmarkResult {
    ...
    if op == "poll" || op == "peek" {
        for i in 0..scale {
            let _ = queue.offer(format!("v{i}")).await;
        }
    }
...
async fn bench_set_op(client: &HazelcastClient, op: &str, scale: usize) -> BenchmarkResult {
    ...
    if op == "contains" || op == "remove" {
        for i in 0..scale {
            let _ = set.add(format!("v{i}")).await;
        }
    }
...
async fn bench_list_op(client: &HazelcastClient, op: &str, scale: usize) -> BenchmarkResult {
    ...
    if op == "get" {
        for i in 0..scale {
            let _ = list.add(format!("v{i}")).await;
        }
    }
...
async fn run_full_benchmark_suite() {
    ...
    macro_rules! run {
        ($r:expr) => {
            let r = $r;
            println!(
                "  {}/{} s={} c={} v={}B: {:.0} ops/s p50={:.2}ms p99={:.2}ms err={}",
                r.data_structure,
                r.operation,
                r.scale,
                r.concurrency,
