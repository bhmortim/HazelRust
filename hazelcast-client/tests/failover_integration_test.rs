...
async fn test_data_ops_after_clear_smoke() {
    ...
    let config = ClientConfigBuilder::new()
        .cluster_name("dev")
        .add_address(common::DEFAULT_CLUSTER_ADDRESS.parse().unwrap())
        .retry(|r| r.max_retries(5).initial_backoff(Duration::from_millis(50)))
        .build()
        .unwrap();
...
async fn test_multiple_data_structures_smoke() {
    ...
    map.put("map-key".to_string(), "map-value".to_string())
        .await
        .unwrap();
    ...
    assert_eq!(
        map.get(&"map-key".to_string()).await.unwrap(),
        Some("map-value".to_string())
    );
...
async fn test_client_lifecycle_smoke() {
    ...
    map.put("before-restart".to_string(), "value".to_string())
        .await
        .unwrap();
...
async fn test_operations_with_high_latency_tolerance() {
    ...
    let config = ClientConfigBuilder::new()
        .cluster_name("dev")
        .add_address(common::DEFAULT_CLUSTER_ADDRESS.parse().unwrap())
        .retry(|r| r.max_retries(5).initial_backoff(Duration::from_millis(200)))
        .build()
        .unwrap();
